...
#define _xd(fmt, ...)	pr_info("BOOT: " fmt, ##__VA_ARGS__)

static void bb_print(int type, const char *buf, ssize_t cnt)
{
	char pr_buf[256], *pp = pr_buf;
	char tmp[32];
	int c, i;

	memset(pr_buf, 0, sizeof(pr_buf));
	if (type)
		pp += sprintf(pp, "write %ld:", (long)cnt);
	else
		pp += sprintf(pp, "read %ld:", (long)cnt);
	c = (cnt < 16) ? cnt : 16;
	...
	size_t count, loff_t *pos)
{
	...
	ret = baseband_ipc_file_write(baseband_usb_chr->ipc,
		file, buf, count, pos);
