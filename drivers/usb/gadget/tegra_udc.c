...
static void asus_cable_detection_work(void)
{
	...
	case CONNECT_TYPE_NONE:
		pr_info("ZZZ: The USB/AC cable is disconnected\n");
		if (project_id == TEGRA3_PROJECT_ME301T) {
			s_cable_info.cable_status = 0x0; //0000
		} else {
			...
			s_cable_info.cable_status = 0x0; //0000
			if (the_udc->vbus_reg)
				regulator_set_current_limit(the_udc->vbus_reg, 0, 0); /* 0A */
			if (project_id != TEGRA3_PROJECT_P1801) gpio_limit_set0_set(0);
		}
		break;
	case CONNECT_TYPE_SDP:
		pr_info("ZZZ: Detected SDP port\n");
		if (project_id == TEGRA3_PROJECT_ME301T) {
			s_cable_info.cable_status = 0x1; //0000
		} else {
			if (adapter_in == 0) {
				pr_info("ZZZ: The USB cable is connected (0.5A)\n");
				s_cable_info.cable_status = 0x1; //0001
				s_cable_info.ac_15v_connected = 0;
				if(project_id != TEGRA3_PROJECT_P1801) gpio_limit_set0_set(0);
			} else {
				pr_info("ZZZ: USB cable + AC adapter 15V connect (1A)\n");
	...
	case CONNECT_TYPE_DCP:
		pr_info("ZZZ: Detected DCP port(wall charger).\n");
		if (project_id == TEGRA3_PROJECT_ME301T) {
			pr_info("ZZZ: AC adapter 5V connect (1A)\n");
			s_cable_info.cable_status = 0x3;
		} else {
			if (dock_in == 0) {//no dock in
				if (adapter_in == 1) {
					pr_info("ZZZ: AC adapter 15V connect (1A)\n");
					s_cable_info.cable_status = 0x3; //0011
					s_cable_info.ac_15v_connected = 1;
					if (the_udc->vbus_reg)
						regulator_set_current_limit(the_udc->vbus_reg, 0, USB_CHARGING_CDP_CURRENT_LIMIT_UA); /* 0.9A */
					if (project_id != TEGRA3_PROJECT_P1801) gpio_limit_set0_set(1);
					...
					pr_info("ZZZ: No define adapter status\n");
					s_cable_info.cable_status = 0x1; //0001
					if (the_udc->vbus_reg)
						regulator_set_current_limit(the_udc->vbus_reg, 0, 0); /* 0A */
					if (project_id != TEGRA3_PROJECT_P1801) gpio_limit_set0_set(0);
...
#endif
					pr_info("ZZZ: %s limt_set1=%d dock_ac=%#X stand_ac=%#X\n", __func__, adapter_in, dock_ac, stand_ac);
					s_cable_info.cable_status = 0x1; //0001
					s_cable_info.ac_15v_connected = 0;

					if (dock_ac == 0x20 || stand_ac > 0x5) {
						pr_info("ZZZ: AC adapter + Docking 15V connect (1A)\n");
						s_cable_info.cable_status = 0x3; //0011
						s_cable_info.ac_15v_connected = 1;
						if (the_udc->vbus_reg)
							regulator_set_current_limit(the_udc->vbus_reg, 0, USB_CHARGING_CDP_CURRENT_LIMIT_UA); /* 0.9A */
						...
						pr_info("ZZZ: AC adapter + Docking 5V connect (1A)\n");
						s_cable_info.cable_status = 0x1; //0001
						s_cable_info.ac_15v_connected = 0;
						if (the_udc->vbus_reg)
							regulator_set_current_limit(the_udc->vbus_reg, 0, 0); /* 0A */
				...
				pr_info("ZZZ: No define the USB status\n");
				if (the_udc->vbus_reg)
					regulator_set_current_limit(the_udc->vbus_reg, 0, 0); /* 0A */
				if (project_id != TEGRA3_PROJECT_P1801) gpio_limit_set0_set(0);
			}
//			if(project_id != TEGRA3_PROJECT_P1801) gpio_limit_set0_set(1);
	...
	case CONNECT_TYPE_CDP:
		pr_info("ZZZ: detected CDP port(1A USB port)\n");
		if (project_id == TEGRA3_PROJECT_ME301T) {
	...
	case CONNECT_TYPE_NON_STANDARD_CHARGER:
		pr_info("ZZZ: detected non-standard charging port\n");
		if (project_id == TEGRA3_PROJECT_ME301T) {
			s_cable_info.cable_status = 0x1;
		} else {
			s_cable_info.cable_status = 0x1; //0001
		...
		break;
	default:
		pr_info("ZZZ: detected USB charging type is unknown\n");
		if (project_id == TEGRA3_PROJECT_ME301T) {
			s_cable_info.cable_status = 0x1;
		} else {
			s_cable_info.cable_status = 0x1; //0001
...
static int tegra_usb_set_charging_current(struct tegra_udc *udc)
{
	...
	case CONNECT_TYPE_NONE:
		pr_info("ZZZ1: detected USB charging is disabled");
		max_ua = 0;
		break;
	case CONNECT_TYPE_SDP:
		pr_info("ZZZ1: detected SDP port");
		max_ua = USB_CHARGING_SDP_CURRENT_LIMIT_UA;
		break;
	case CONNECT_TYPE_DCP:
		pr_info("ZZZ1: detected DCP port(wall charger)");
		max_ua = USB_CHARGING_DCP_CURRENT_LIMIT_UA;
		break;
	case CONNECT_TYPE_CDP:
		pr_info("ZZZ1: detected CDP port(1A USB port)");
		max_ua = USB_CHARGING_CDP_CURRENT_LIMIT_UA;
		break;
	case CONNECT_TYPE_NON_STANDARD_CHARGER:
		pr_info("ZZZ1: detected non-standard charging port");
		max_ua = USB_CHARGING_NON_STANDARD_CHARGER_CURRENT_LIMIT_UA;
		break;
	default:
		pr_info("ZZZ1: detected USB charging type is unknown");
		max_ua = 0;
	}
...
static int tegra_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	if (udc->vbus_active && !is_active) {
		...
		udc->ep0_dir = 0;
		udc->vbus_active = 1;
		if (gpio_get_value(TEGRA_GPIO_PU4)) { /* dock not connected */
